...
import BkCore from "../../lib/utils/BkCore";

export default class BkClientCore extends BkLib {
  constructor() {
    super();
    //#################################      DEVICE INFO      ##################################
    this._device = {
...
//        $(e.target.form).find('[name='+field+']').val("")
  ...
  // DEPRECATED
  static callDom(e, DomId, options, parentDom) {
    e.preventDefault();
    let data={};
    //parentDom = undefined
    if ($('#'+parentDom+'-container')[0] !== undefined) { $('#'+parentDom+'-container').remove(); }
  ...
  static callFieldValidation(event, template, silent) {
    let value;
    ...
    if (true) {
      ...
      if (pref.type === "enumstring_many") { checked = $(event.target).prop("checked"); }
      if (silent) {
        ...
        if (pref.type !== 'files') { model.set(field,value,checked); }
      }
    }
    if (model.isPersisted()) { template.userValue = model.get(field); }
    if (!context.noValidation) {
      if (model.isValid(field)) {
        //must empty input if model valid for array
        if (fieldType === 'array') { return event.value = ""; }
      } else {
  ...
  static getResizedPicture(img, profile) {
    let maxH, maxW;
  ...
  static callFormInline(e, context) {
    let field;
  ...
  static callModal(e, DomId, options, parentDom) {
    e.preventDefault();
    let data={};
  ...
  static callModelAction(event, templateOrModel, options) {
    let model;
    ...
    if (event !== undefined) { options.event = event; }
...
// Give external representation of the value in view or input tags (following tag parameter)
  static getValueForField(context,tag) {
    ...
    if (context._pref != null) {
      ...
      pref = (context._pref = BkCore.getPreferenceFields(model,field));
    }
    const {
      _export
    } = context;
    //pref = {type: context.type} if field in ['creationDate','updateDate'] and tag is 'view'

    if (!model || !field || !pref) { return; }

    let valueType = pref.type;
    ...
    switch (valueType) {
      ...
      case 'enumstring_many':
        ...
        break;
      case 'markdown':
        if ((typeof value === "string") && (tag === "view")) { res = markdown.toHTML(value); }
        break;
      case 'editor':
        if ((typeof value === "string") && (tag === "view")) { res = markdown.toHTML(value); }
        break;
      case 'color':
        if (tag === "input") { res = pref.colors; }
        break;
      //when 'file'
      //  if tag is "input"
      ...
      case 'hash':
        if (pref !== undefined) { return (Array.from(_.without(_.keys(pref),"type")).map((subfield) => field + "." + subfield)); }
        break;
      case 'array':
        if (value === undefined) {
          return undefined;
      ...
      case 'belongs_to':
        if ((tag === "view") || pref.autocomplete) {
          ...
          if (currentView.isRendered) {
            ...
            let i = 0;
            while ((i < value.length) && !res[i].selected) { i++; }
  ...
  static getBelongsToStringValues(model, field, currentValue, context) {
    let m;
    const pref = context._pref;
    //   Allow possibility to use allowBlank directly in the pref attributes. Very usefull for fieldList
    if (_.isUndefined(context.allowBlank)) { context.allowBlank = pref.allowBlank; }
    let valuesArray = ((() => {
      const result = [];
      for (m of Array.from(model.getAll(field))) {         result.push({label: m.name , value: m.id, selected: m.id === currentValue });
      }
      return result;
    })());
    ...
    if ((!pref.optional && !context.allowBlank) || (!context.allowBlank && !_.isUndefined(context.allowBlank))) {
      ...
      valuesArray = _.union([{label: I18n.t("app.undefined"), value: "", selected: _.isUndefined(currentValue) }],valuesArray);
    }
    return valuesArray;
  }

  static getEnumStringManyValues(model,field,currentValue,context) {
    let valuesArray;
    try {
      let currentValues;
      const pref = context._pref;
      ...
      valuesArray =  (Array.from(values).map((value) => ({label: model._type + ".enum_string." + field + "." + value, value, selected: _.contains(currentValues,value) })));
    } catch (e) {
      return [{label: "You have to define values in " + model._type + " @_fields definition for enumstringMany field " + field, value: 0}];
    }
    return valuesArray;
  }

  static getEnumStringValues(model,field,currentValue,context) {
    let valuesArray;
    try {
      const pref = context._pref;
      let {
      ...
      if (typeof values === "function") { values = values(model.attributes); }
      valuesArray =  (Array.from(values).map((value) => ({label: model._type + ".enum_string." + field + "." + value, value, selected: value === currentValue})));
      if (pref.optional || context.allowBlank) { valuesArray = _.union([{label: "app.undefined", value: "", selected: _.isUndefined(currentValue) }],valuesArray); }
      if (!pref.optional && !context.allowBlank && _.isUndefined(currentValue && !_.isEmpty(valuesArray))) {
        model.set(field,valuesArray[0].value);
        valuesArray[0].selected = true;
      }
    } catch (e) {
      return [{label: "You have to define values in " + model._type + " @_fields definition for enumstring field " + field, value: 0}];
    }
    return valuesArray;
  }

  static getFormFiles(klass) {
  ...
  static getHasManyToStringValues(model, field, currentValue, context) {
    const pref = context._pref;
    let valuesArray = (Array.from(model.getAll(field)).map((m) => ({label: m.name , value: m.id, selected: m.id === currentValue })));
    if (_.isEmpty(currentValue) || pref.optional || context.allowBlank) { valuesArray = _.union([{label: I18n.t("app.undefined"), value: "", selected: _.isUndefined(currentValue) }],valuesArray); }
    return valuesArray;
  }

  static getLifeCycleValues(model,field,currentValue,options) {
    let valuesArray;
    try {
      const values = model.constructor[field];
      valuesArray =  (Array.from(values).map((value) => ({label: I18n.t(model._type + ".lifecycle." + value), value, selected: value === currentValue })));
      if (options.allowBlank) { valuesArray = _.union([{label: I18n.t("app.undefined"), value: "", selected: _.isUndefined(currentValue) }],valuesArray); }
    } catch (e) {
      return [{label: "You have to define values in " + model._type + " @field_Name definition for lifecycle field " + field, value: 0}];
    }
    return valuesArray;
  }

  static getMarkdownToHTML(o) {
  ...
  static getRadioValues(model,field,currentValue,context) {
    let valuesArray;
    try {
      const pref = context._pref;
      let {
      ...
      if (typeof values === "function") { values = values(model.attributes); }
      valuesArray =  (Array.from(values).map((value) => ({label: model._type + ".radio." + field + "." + value, value, selected: value === currentValue})));
    } catch (e) {
      return [{label: "You have to define values in " + model._type + " @_fields definition for radio field " + field, value: 0}];
    }
    return valuesArray;
  }

  static getValueForBelongsTo(model, field, value, context) {
    if (!_.isEmpty(value)) {
...
//      relation.name
  ...
  static getValueForBelongsToMany(model, field, values, context) {
    if (!context.count) {
  ...
  static getValueForEnumStr(modelName, value, field) {
    if (!_.isUndefined(value)) {
      return modelName + ".enum_string." + field + "." + value;
    }
  }

  static getColor(letter, colorsHash) {
