...
import { AstronomyClasses, Enum, Amount, Textarea } from "../modules/customFields/module.js";
import {XlsExportColumnParam} from "../lib";
...
ParameterTableElement.createChild = function(parameterTable) {
  ...
  if (Class.has(childName)) {
    Class.classes[childName] = undefined
  ...
  child.getConfiguration = function() {
  ...
  parameterTable.options.forEach((option) => {
    let fieldType, template, placeholder, sort
    switch (option.inputType) {
      ...
      case "Amount":
        fieldType = Amount
        break
      case "XlsExportColumnParam":
        if (option.multipleChoice)
          fieldType = [XlsExportColumnParam]
        else
          fieldType = XlsExportColumnParam
