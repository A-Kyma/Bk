...
// Creation of a table object, with options from table tag
  constructor(options) {
    let f, fieldsArray, label, pref;
    if (options == null) { options = {}; }
    const self = this;

    this.perPage = Number(options.perPage);
    if (_.isNaN(this.perPage) || (this.perPage < 1)) { this.perPage = 20; }
    this.filters = {};
    this.defaultFilter = {};
    this.defaultSort = {};
    ...
    if (options.array) {
      ...
      this.model = options.array._type || options.model;
      this.klass = global[this.model];
      this.type = "array";
    } else {
      if (typeof options.model !== "string") {
        this.model = options.model && options.model._type;
      } else {
    ...
    this.filterModel = this.klass && this.klass.new();

    if (options.fields != null) {
      fieldsArray = options.fields.replace(RegExp(" ", "g"), "").split(",");
    } else {
      fieldsArray =  this.klass.getFilterFields(); // _.keys(@klass._fields)
    }

    const fieldsEditable = (options.editableFields && options.editableFields.replace(RegExp(" ", "g"),"").split(",")) || [];
...
//        label = @model + "." + f + ".label"
    ...
    if (typeof options.filter !== "string") {
      options.filter = EJSON.stringify(options.filter);
    }
    if (typeof options.sort !== "string") {
      self.setSort(options.sort);
    } else {
      try {
        self.setSort(EJSON.parse(options.sort) || {});
      } catch (error) {}
    }
    ...
    if (_.isUndefined(options.array)) {
      if (options.filter !== undefined) {
        if (options.filter.indexOf('currentUser') !== -1) {
          options.filter = options.filter.replace('currentUser','\"'+Meteor.userId()+'\"');
        }
        if (options.filter.indexOf('myRoleActivity') !== -1) {
          options.filter = options.filter.replace('myRoleActivity',EJSON.stringify(Role.activity_ids(Meteor.userId())));
        }
      }
      try {
        self.defaultFilter = EJSON.parse(options.filter) || {};
      } catch (error1) {}
    }
...
//         self.handler and self.handler.stop() #solve issue second page display first before refreshing
  ...
  setFilter(filter,options) {
    ...
    for (f in (filter)) {
      ...
      if (!_.isUndefined(this.filterModel._fieldsToInit[f]) && !options.replace) {
        for (let i of Array.from(this.filterModel._fieldsToInit[f])) {
          const h = {};
          h[i] = "";
          this.setFilter(h,options);
        }
      }
    }
    this.full = false;
...
//   Subscription based datatable
    let array;
    if (options == null) { options = {}; }
    const modelize = options.modelize || (options.hash && options.hash.modelize);
    ...
    if (this.type === "subscription") {
      if (this.handler && (this.handler.ready() || (this.scroll && (this.page > 1)))) {
        ...
        if  (!this.multi && !this.full && !this.scroll && !(this.page <= 1)) {
          const nbr = this.klass.count(selector);
          if (nbr > this.perPage) {
            options.skip = nbr - (nbr%this.perPage);
          }
        }
        if (modelize) {
          array = this.klass.toArray(selector,options);
        } else {
          array = this.klass.where(selector,options);
        }
      } else {
        array = [];
      }
...
//   Model with full autopublish model or subscribed during route initialization
      ...
      if (modelize) {
        array = this.klass.toArray(this.getSelector(),this.getOptions(),suboptions);
      } else {
        array = this.klass.where(this.getSelector(),this.getOptions(),suboptions);
      }
    }
//   Do we expect all records we can see from pub/sub are viewable ?
    ...
    if ((this.type === "subscription") && this.handler) {
      ...
      return autorun = Tracker.autorun(function() {
        ...
        if (self.handler.ready() && self.full) {
          callback(self.klass.toArray(self.getSelector()));
          autorun.stop();
          return;
        }
        if (!self.full) {
      ...
      return callback(this.klass.toArray(this.getSelector()));
    }
  }
  ...
  getCount() {
    ...
    if ((this.type === "subscription") && this.handler) {
      ...
      return global[this.model].count(this.getSelector(),this.getOptions(true));
    }
  }
  ...
  exportToCsv() {
    ...
    return this.getFullArray(function(array) {
      for (let model of Array.from(array)) {
        for (df of Array.from(exportFields)) {
          const pref = BkCore.getPreferenceFields(model,df.field);
          const context = {
            model,
            field: df.field,
            _pref: pref,
          ...
          const value = BkClientCore.getValueForField(context,"view");
          const externalValue = pref.type === "enumstring" ? I18n.t(value) : value;
          bb = bb + '"' + externalValue + '";';
        }
        bb += "\n";
      }
      ...
      return saveAs(blob,self.getTitle() + " - " + I18n.getDate(new Date) + "@" + I18n.getTime(new Date) + ".csv");
    });
  }
}
Datatable.initClass();

// This method will allow to publish easily any collection in order to make it compatible with datatable.
// Arguments are:
// - subscriptionName : name of the subscription the will be used in tag datatable.
// - collection : the Meteor collection behind this publication
// - restrictionFunction : the clause added in the selector to avoid retrieve unauthorized collections
...
if (Meteor.isServer) {
  Datatable.publish = (subscriptionName, collection, restrictionFunction) => Meteor.publish(subscriptionName, function(selector,options) {
    if (selector == null) { selector = {}; }
    if (!this.userId) { return []; }
    const restriction = (restrictionFunction && restrictionFunction(this.userId)) || {};
    if (restriction) { selector = {"$and": [restriction,selector]}; }
    publishCount(this, subscriptionName + '-count', collection.find(selector, { fields: { _id: true }}), { noReady: true });
    return [ collection.find(selector,options), Filters.find({creationUser: this.userId, subscription: subscriptionName},{sort: {name: 1}}) ];
  });
}
