...
// Replaced - sort: default order used for the subscription. Must be in format: sort='{"field": 1}'
...
// Creation of a table object, with options from table tag
  constructor(options) {
    ...
    if (options.array) {
      ...
      this.klass = options.model.constructor;
      this.model = this.klass.getName();
      this.type = "array";
    }
    ...
    if (options.fields != null) {
      ...
      fieldsArray =  keys(this.klass.schema.fields);
    }
    ...
    if (this.type === "subscription") {
      if (this.handler && (this.handler.ready() || (this.scroll && (this.page > 1)))) {
        ...
        if (modelize) {
          ...
          array = this.klass.find(selector,options).fetch();
        }
      } else {
        array = [];
      }
...
//   Model with full autopublish model or subscribed during route initialization
      ...
      if (modelize) {
        ...
        array = this.klass.find(this.getSelector(),options).fetch();
      }
    }
 ...
 * Arguments are:
 * @param subscriptionName - name of the subscription that will be used in tag datatable.
 * @param collection - Mongo.collection behind this publication
 * @param isPublic - if isPublic, then userId not verified
 * @param restrictionFunction - the clause added in the selector to avoid retrieve unauthorized collections
 ...
 ***/
  Datatable.publish = (subscriptionName, collection, isPublic, restrictionFunction) => Meteor.publish(subscriptionName, function(selector,options) {
    if (selector == null) { selector = {}; }
    if (!isPublic && !this.userId) { return this.ready(); }
