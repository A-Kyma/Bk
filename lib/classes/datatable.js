...
// - sortValue : For type Array: contains evaluation function by field in order to sort the field :
...
// - exportType: XLSX / PDF / CSV (default)
// - exportMethod: Meteor Method to retrieve the JSON data for XLSX
...
class Datatable {
  static initClass() {
    ...
    this.prototype.updateRoute = false
  }
...
// Creation of a table object, with options from table tag
  constructor(vueComponent) {
    ...
    if (options.sortValue) { this.sortValue = options.sortValue }
    ...
    if (typeof options.model === "string") {
      ...
      this.klass = Class.getClass(options.model);
    } else {
    ...
    if (options.fields != null) {
      ...
      fieldsArray =  this.klass.getFieldsNamesByFilter(options)
    }
    ...
    let subscribeFieldsArray = []
    if (Array.isArray(options.subscribeFields)) subscribeFieldsArray = options.subscribeFields
    else if (options.subscribeFields) subscribeFieldsArray = options.subscribeFields.replace(RegExp(" ","g"),"").split(",") || []
...
//        label = @model + "." + f + ".label"
      ...
      exportFieldsHash[f].format = pref?.exportFormat
    }
    ...
    */
    ...
    let filterClass = options.filterClass || (this.klass && this.klass.createFilterClass())
    this.filterModel = filterClass && new filterClass(self.defaultFilter);
    ...
    if (!_.isUndefined(options.initialFilter)) {
      ...
      self.setFilter(options.initialFilter, {initial: true});
    }

    if (this.updateRoute) {
      this.restoreFilterFromRoute()
      this.restorePageFromRoute()
    }
    else this.filters = this.filterModel.getFilterQuery()
    ...
    if (options.subscription) {
      ...
      const suboptions = {};
      if ((options.subscribeFields !== "*" && options.subscribeFields !== undefined)) {
        suboptions.fields = fieldsSelected
      }
...
//         self.handler and self.handler.stop() #solve issue second page display first before refreshing
        ...
        let subscriptionParams = options.subscriptionParams
        self.handler=Meteor.subscribe(self.subscription,self.getSelector(),suboptions,subscriptionParams);
        Tracker.autorun(() => {
          if (self.handler.ready()) {
            self.readydep.changed()
  ...
  setPage(p) {
    ...
    if (p > m) { p = m; }
    this.page = p;
    if (this.updateRoute && this.vueComponent){
      let route = this.vueComponent.$route
      let router = this.vueComponent.$router
      if (parseInt(route?.query?.currentPage) !== this.page && this.page !== null){
        router.replace({name: route.name, query: {...route.query, currentPage: this.page}})
      }
    }
    this.subdep.changed();
  ...
  /***
   * @summary Set new filter
   * @param filter {Object} - New filter to set
   * @param options {Object} - Options for setting filter
   * @param options.replace {Boolean} - replace entire filter with new one
   * @param options.initial {Boolean} - initial filter: do not replace existing filter
   */
  setFilter(filter,options) {
    ...
    if (options.initial) {
      if (this.updateRoute && this.vueComponent) {
        ...
        let params = encodeURI(EJSON.stringify(this.filterModel.raw()))
        router.replace({name: route.name, query: {...route.query, filter: params}}).catch((e) => {
          if (!(e.message =~ /^Avoided redundant navigation/)) throw(e)
        })
      }
    } else {
      this.applyFilter()
  ...
  restorePageFromRoute() {
    if (!this.updateRoute || !this.vueComponent) return
    let route = this.vueComponent.$route
    if (!route.query || !route.query.currentPage) return
    let page = Number.parseInt(route.query.currentPage)
    this.page = page
  }
  ...
  restoreFilterFromRoute() {
    ...
    if (!route.query || !route.query.filter) {
      this.filters = this.filterModel.getFilterQuery()
      return
    }
    let params
    try {
      params = EJSON.parse(decodeURI(route.query.filter))
    } catch(e) {
      return
    }
    this.filterModel = new (this.filterModel.constructor)({...params,...this.defaultFilter})
    this.filters = this.filterModel.getFilterQuery()
  }
      ...
      if (this.handler && (this.handler.ready() || (this.scroll && (this.page > 1)))) {
        ...
        //Tracker.autorun(() => {
            if (self.handler.ready()) {
              ...
              array = self.klass.find(selector,options).fetch()
              this.array = array
            } else {
              array = this.array
            }
      ...
      if (options && options.sort && Object.entries(options.sort).length > 0) {
        array.sort((a,b) => {
          for (let [sortField, sortOrder] of Object.entries(options.sort)) {
            let sortEvalFunc = this.sortValue[sortField]
            let valueA
            let valueB
            if (typeof sortEvalFunc === "function") {
              valueA = sortEvalFunc(a)
              valueB = sortEvalFunc(b)
            } else {
              valueA = (Class.includes(a.constructor)) ? a.getValue(sortField) : a
              valueB = (Class.includes(b.constructor)) ? b.getValue(sortField) : b
            }
...
//   Model with full autopublish model or subscribed during route initialization
      ...
      this.array = array = self.klass.find(self.getSelector(),options).fetch()
    }
    ...
    if ((this.type === "subscription") && this.handler) {
      ...
      return autorun = Tracker.autorun(function() {
        ...
        if (self.handler.ready() && self.full) {
          callback(self.klass.find(self.getSelector(), {}));
          autorun?.stop();
          return;
        }
        if (!self.full) {
...
if (Meteor.isServer) {
   ...
   * @requires meteor/akyma:publish-counts
   ***/
