 ...
 * @param {String} access one of "SuperAdministrator", "Administrator", "Member", "User"
 * @param {Class} model Astronomy class instance
 * @param {String|Class} user user for which we check
 * @returns {true|false}
 ***/
Role.is = function (access, model, user, options={}) {
  if (!user) {
    try {
      user = Meteor.userId()
    }
    catch (e) {
      // It's a call initiated by server (i.e. scheduler or Meteor.setTimeout
      if (Meteor.isServer && DDP._CurrentInvocation.get() === null)
        return true
    }
  }
