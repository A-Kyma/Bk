...
I18n.locales = config.translation.locales;
let defaultLanguage = config.translation.defaultLanguage;
if (Meteor.isClient) {
...
I18n.get = function(key,options={}) {
  ...
  if (Array.isArray(options.param) && options.doc && options.nestedName) {
    let classDefinition = options.doc.getClass(options.nestedName);
    if (["Enum","Lifecycle"].includes(classDefinition.name)) {
      let param = [];
