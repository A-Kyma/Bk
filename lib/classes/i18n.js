...
import {DateTime, Languages} from "../modules/customFields/module.js";
...
function replaceLocaleVariable(result,options) {
  ...
  if (variableKeys) {
    variableKeys.forEach((elem) => {
      ...
      result = result.replace(new RegExp("\\{\\{\\\$"+k.slice(1)+"\\}\\}","g"),value)
    })
  }
  return result
}
...
function replaceContext(result,askedKey) {
  ...
  if (Object.entries(contextKeys).length === 0) {
    ...
    return result.replace(new RegExp("\\{\\{context\\.\\w+\\}\\}","g"),value)
  }
