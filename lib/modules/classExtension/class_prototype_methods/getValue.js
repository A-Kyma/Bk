...
import {Class,ListField,ObjectField} from "meteor/akyma:astronomy"

function getValue(field, format, unit) {
  ...
  if (definition.type?.name === "Amount"){
    return fieldClass.getValue(value,definition.decimal,unit,definition.currency)
  }
  if (definition.type?.name === "Number"){
    return fieldClass.getValue(value,definition.decimal)
  }
  if (definition.type?.name === "Percentage") {
    return fieldClass.getValue(value)
  }
  if (["Date","DateTime"].includes(definition?.type?.name)){
        if (extFormat === "relative"){
          return DateTime.relative(value)
        }
        return fieldClass.getValue(value,extFormat)
  }
  if (["Number","Integer","String"].includes(definition?.type?.name)){
      return (definition.ui?.append)? value + definition.ui.append : value
  }
  // /!\ fieldClass.name is shortened in production, so not = "Enum" or "Lifecycle"
  ...
  let fieldClassName = fieldClass?.getClassName && fieldClass?.getClassName()
  if (["Enum","Lifecycle"].includes(fieldClassName)) {
    if (definition instanceof ListField) {
