...
import { ObjectField, ListField } from 'meteor/akyma:astronomy';
...
function onApplyDefinition(Class, parsedDefinition, className) {
  ...
  _each(Class.schema.fields,(fieldDefinition, fieldName) => {
    ...
    if (!fieldDefinition.default) {
      ...
      if (Lifecycle.includes(fieldClass) && fieldClass.getDefault() !== undefined) {
          fieldDefinition.default = fieldClass.getDefault()
      }
    }

    // We create transitions for lifecycle
    if (Lifecycle.includes(fieldClass)) {
      fieldClass.createTransitionsForPrototype(Class.prototype,fieldName)
    }
