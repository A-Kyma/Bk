...
import { Type, Validators, Class, reservedKeywords } from 'meteor/akyma:astronomy';
  ...
  return function (options = {}, callback) {
    ...
    */
    ...
    this.save(options, (err, result) => {
      ...
      Match.test(callback, Function) && callback(err, result, this)
    })
  }
}
...
class Lifecycle {
  ...
  static get(lifecycleName) {
    return this.lifecycles[lifecycleName];
  }
  ...
  static create(definition) {
    ...
    Lifecycle.getTransitionsForRole = function(model,from,role,field,to) {
      ...
      let transitionValid = function(t) {
        ...
        if (Match.test(t.condition,Function) && !t.condition({doc: model, from})) {
          return false
        }
    ...
    Lifecycle.createTransitionsForPrototype = function(prototype,field) {
      ...
      Object.keys(transitions).forEach(transition => {
        ...
        if (reservedKeywords.includes(transition))
          throwParseError([{
            'class': prototype.constructor.getName()
          }, {
            'Lifecycle field class': Lifecycle.getName()
          },{
            'transition': transition
          },
            `Lifecycle cannot create a transition in reserved keywords : ${transition}`
          ]);
