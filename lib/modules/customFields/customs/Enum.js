/***
 * Same as original akyma:astronomy, but only accept Array identifiers
  ...
  create(definition) {
    ...
    //   const I18n = I18nModule.default;
      Enum.getOptions = function(opts={}) {
        ...
        _defaults(opts, {optional: false, sort: false})

        let options = Enum.getIdentifiers().map(x => {
          let key = Enum.getLabelKey(x);
            return {text: I18n.get(key,opts), key, value: x}
        })

        if (opts.optional) {
          let key = "app.undefined"
          options.splice(0, 0, {"text": I18n.get(key,{locale:opts.locale}), key, value: null})
        }
      ...
      Enum.getOptionLabel = function(identifier,locale) {
        let key = Enum.getLabelKey(identifier)
        if (key.includes(".")) {
          const I18n = Class.get("I18n")
