...
class DateTime extends Date {
   ...
   * @param {String} format - see https://moment.github.io/luxon/#/parsing?id=table-of-tokens
   * @returns {LuxonDateTime} Luxon DateTime
   */
  static getDateTimeFromTimezone(date,timeZone="Europe/Brussels", format) {
    let datetime
    if (date instanceof Date)
      datetime = LuxonDateTime.fromJSDate(date, {zone: timeZone})
    else
      if (format)
        datetime = LuxonDateTime.fromFormat(date, format, {zone: timeZone})
      else
   ...
   */
  static createDateFromTimezone(date,timeZone,format) {
    let datetime = this.getDateTimeFromTimezone(date,timeZone,format)
